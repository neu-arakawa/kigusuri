...
<OPTION VALUE=\"$page:=:$goods_id:=:$name:=:$price:=:10\">10</OPTION>
		...
		print "<form method=\"post\" action=\"$cgifile\">\n";
		print "<table bgcolor=\"#333333\" cellpadding=\"0\" cellspacing=\"0\" width=\"650\" border=\"0\" class='table-shop'>\n";
		print "<tr><td>\n";
		print "<table cellpadding=\"1\" cellspacing=\"1\" width=\"650\" border=\"0\" class=\"lineup\">\n";
		print "<tr class=\"main2\" bgcolor=\"#caed54\"><td class='name-shop'><b>$name</b></td></tr>\n";
		
		if(($comment ne '') or ($picture ne '')){
			print "<tr class=\"main2\" bgcolor=\"#ffffcc\"><td>\n";
			print "<table border=0 width=\"100%\"><tr>\n";
			if($comment ne ''){ print "<td valign=\"top\" class='comment-shop'>$comment</td>"; }
			if($picture ne ''){ 
				$ImgSize = '';
				($Wi,$He) = &Get_JpegSize($picture);
				if($Wi > 200 or $He > 200){ $ImgSize= 'width="200"' }
				print "<td valign=\"top\" align=\"center\"><a href=\"pic/$picture\" target=\"_blank\"><IMG SRC=\"pic/$picture\" border=\"0\" width=\"200px\"></a></td>\n";
			}
...
##		print "<TR class=\"main\"><TD>$comment\n";
		...
		if ($zaiko_disp eq "on") {
			...
			if ($limit <= "0") {
				...
				#print "<TR class=\"main2\" BGCOLOR=ffffcc><TD align=right class='price-shop'>";
				#print "<FONT COLOR=\"#000000\"><B>税込価格 $price_disp 円</B></FONT> $selectgoods <INPUT TYPE=\"SUBMIT\"NAME=\"cart\"VALUE=\"カー\トに入れる\">\n";
...
<OPTION VALUE=\"$page:=:$goods_id:=:$name:=:$price:=:10\">10</OPTION>
		...
		if(($comment ne '') or ($picture ne '')){
			...
			if($comment ne ''){ print "<td width=400 class='comment-shop'>$comment</td>"; }
			if($picture ne ''){ 
				$ImgSize = '';
				($Wi,$He) = &Get_JpegSize($picture);
