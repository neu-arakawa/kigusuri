...
#	}
	...
	if ($memory eq on) {
		...
		print "<br>E-mail登録<input type=\"checkbox\" name=\"memo_check\"value=\"on\"><br>";
		print '<font size=2>※E-mail登録をＯＮにされますとお届け先情報を記録され、<br>　 次回購入時に上部の【登録メールアドレス】に<br>　 メールアドレスをご入力いただくだけで、<br>　 お届け先情報が自動で入力されます。</font>' . "</TD>\n";
		print "</TR>\n";
	} else {
		print "<TR BGCOLOR=\"$tab_color_main\" class=\"main2\">\n";
...
sub pass {
	...
	if ($regname eq "" || $kana eq "" || $email eq "" || $add eq "" || $tel eq ""  || $shiharai eq "") {
		&error("ERROR","お名前、フリガナ、E-mail、ご住所、お電話番号、お支払方法 は必須項目です。","BACK");
		exit;
	}
