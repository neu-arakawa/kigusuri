...
# Disp_Html			Htmlファイル表示			$errFlg = &Disp_Html($FileName,$NoCacheFlg);
...
# Get_JpegSize		Jpeg画像のサイズ取得		($W,$H) = &Get_JpegSize($FileName);
...
sub Get_Time{
	...
	$mon++;
	$year += 1900;
	unless($format){ $format = 'yyyy/mm/dd'; }
	if($format eq 'yyyy/mm/dd')       { return sprintf("%04d/%02d/%02d", $year, $mon, $mday); }
	if($format eq 'yyyy/mm/dd hh:mm') { return sprintf("%04d/%02d/%02d %02d:%02d", $year, $mon, $mday, $hour, $min); }
	if($format eq 'yyyymmddhhmmss')   { return sprintf("%04d%02d%02d%02d%02d%02d", $year, $mon, $mday, $hour, $min, $sec); }
	if($format eq 'mmddhhmmss')       { return sprintf("%02d%02d%02d%02d%02d",  $mon, $mday, $hour, $min, $sec); }
	if($format eq 'yyyymmdd')         { return sprintf("%04d%02d%02d", $year, $mon, $mday); }
}
...
sub Send_Mail{
	...
	close(TEMP);
	($mailh,$mailb) = split('---- Border Line ----',$maildata);
#	return("$mailh,$mailb");
	...
	## ボディ作成
	&jcode::convert(\$mailb, "jis");
	
	
	## メール送信
	if(!open(MAIL, "|$sendmail $l_mail")) { $errflg = 2; return($errflg); }
		print MAIL $mailh;
	...
	# メール送信
#	if(!open(MAIL, "|$sendmail $l_mail")) { $errflg = 1; return($errflg); }
#	if(!open(TEMP, "$l_mailform"))        { $errflg = 1; return($errflg); }
#		while (<TEMP>) {
#			if(index($_,'Subject:') >= 0){
#				s/Subject\: //;
