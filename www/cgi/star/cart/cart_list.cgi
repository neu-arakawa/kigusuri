...
<OPTION VALUE=\"$page:=:$goods_id:=:$name:=:$price:=:10\">10</OPTION>
		...
		print "<form method=\"post\" action=\"$cgifile\">\n";
		print "<table bgcolor=\"#333333\" cellpadding=\"0\" cellspacing=\"0\" width=\"650\" border=\"0\">\n";
		print "<tr><td>\n";
		print "<table cellpadding=\"1\" cellspacing=\"1\" width=\"650\" border=\"0\" class=\"lineup\">\n";
		print "<tr class=\"main2\" bgcolor=\"#caed54\"><td style='padding:6px 12px;'><b>$name</b></td></tr>\n";
		
		if(($comment ne '') or ($picture ne '')){
			print "<tr class=\"main2\" bgcolor=\"#ffffcc\"><td>\n";
			print "<table border=0 width=\"100%\"><tr>\n";
			if($comment ne ''){ print "<td style='padding:10px;'>$comment</td>"; }
			if($picture ne ''){ 
				$ImgSize = '';
				($Wi,$He) = &Get_JpegSize($picture);
				if($Wi > 200 or $He > 200){ $ImgSize= 'width="240"' }
				print "<td valign=\"top\" align=\"center\"><a href=\"pic/$picture\" target=\"_blank\"><IMG SRC=\"pic/$picture\" border=\"0\" width=\"200px\" style='margin:10px'></a></td>\n";
			}
...
<OPTION VALUE=\"$page:=:$goods_id:=:$name:=:$price:=:10\">10</OPTION>
		...
		if(($comment ne '') or ($picture ne '')){
			...
			if($comment ne ''){ print "<td width=400 style='padding:10px;'>$comment</td>"; }
			if($picture ne ''){ 
				$ImgSize = '';
				($Wi,$He) = &Get_JpegSize($picture);
				if($Wi > 240 or $He > 180){ $ImgSize= 'width="240" height="180"' }
				print "<TD VALIGN=TOP align=center><IMG SRC=\"pic/$picture\" BORDER=0 $ImgSize style='margin:10px;'></td>\n";
			}
			print "</tr></table></td></tr>\n";
		}
...
sub reg2{
	...
	print "<TD>\n";
	print "<TABLE CELLPADDING=2 CELLSPACING=1 WIDTH=600 BORDER=0 class=\"orderf\">\n";
	...
	if ($memory eq on) {
		...
		print "<TR BGCOLOR=\"#333333\">\n";
		print "<TD>\n";
		print "<TABLE CELLPADDING=2 CELLSPACING=1 WIDTH=500 BORDER=0 class=\"orderf\">\n";
	...
	print "<TABLE BGCOLOR=\"#FFFFFF\" CELLPADDING=0 CELLSPACING=0 WIDTH=600 BORDER=0 class=\"orderf\">\n";
...
#	}
	...
	print "<TR>\n";
	print "<TD><TABLE CELLPADDING=2 CELLSPACING=1 WIDTH=600 BORDER=0 class=\"orderf\">\n";
	...
	print "</TR>\n";
	print "<TR BGCOLOR=\"$tab_color_main\" class=\"main2\">\n";
	print "<TD>ふりがな<span style=\"color:red;\"> *</span></TD><TD><input type=text name=kana size=30>　<font size=2>※ひらがなでご記入ください。</font></TD>\n";
	print "</TR>\n";
	if ($memory eq on) {
		print "<TR BGCOLOR=\"$tab_color_main\" class=\"main2\">\n";
		...
		print "<span style=\"font-size:0.8em;\">※アドレスを登録していただくと、次回購入時に、<br>　「ご購入者 連絡先」のご入力が不要になります。</span></p>" . "</TD>\n";
		print "</TR>\n";
	} else {
		print "<TR BGCOLOR=\"$tab_color_main\" class=\"main2\">\n";
	...
	print "</TR>\n";
	print "<TR ALIGN=CENTER class=\"main2\">\n";
	print "<TD COLSPAN=2 BGCOLOR=\"$tab_color_top\">お届け先が違う場合のみ ご記入下さい。</TD>\n";
	print "</TR>\n";
	...
	print "<TABLE  BGCOLOR=\"#333333\" CELLPADDING=0 CELLSPACING=0 WIDTH=0 BORDER=0>\n";
	print "<TR>\n";
	print "<TD><TABLE CELLPADDING=2 CELLSPACING=1 WIDTH=500 BORDER=0 class=\"orderf\">\n";
	print "<TR ALIGN=CENTER class=\"main2\">\n";
...
sub pass {
	...
	if ($regname eq "" || $email eq "" || $add eq "" || $tel eq ""  || $shiharai eq "") {
		&error("ERROR","\n<br><br><b style=\"color:red\">お支払方法、お名前、ふりがな、E-mail、ご住所、お電話番号 は必須項目です。</b><br>\n","BACK");
		exit;
	}
