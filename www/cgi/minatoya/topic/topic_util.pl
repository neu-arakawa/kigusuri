...
# FileReadMatch		マッチするファイル内容取得	@rdata  = &FileReadMatch($FileName,$data,$colnumber);
...
sub Chk_Pwd{
	...
	## パスワードの取得
	if($AuthenDataFile eq ''){ $AuthenDataFile = '../authen/authen_data.pl'; }
	my $adata = &FileReadMatch($AuthenDataFile,$id,1);
	($Authen{'id'},$Authen{'pwd'},$Authen{'name'}, $Authen{'tname'},$Authen{'renraku'},$Authen{'group'} ) = split($SPC,$adata);
	chomp($Authen{'group'});
	
	
	if ($pwd ne $Authen{pwd}){
		exitError("ＩＤまたはパスワードが間違っています。");
	}
...
sub Get_Irno{
	...
	local $fnamer = $IRNO_SEQr;
	local $fnamew = $IRNO_SEQw;
	local $retcnt;
...
sub FileReadMatch {
    ...
    while (<FH>) {
        @fdata = split($SPC);
		if($data eq $fdata[$colnum]){ return($_); }
    }
    close(FH);
    return(1);
